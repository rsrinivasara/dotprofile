...
class NeovimEnv(object):
    ...
    def _setup_venv(self, venv_path, python_exec):
        ...
        if not os.path.exists(venv_path):
            print('Creating venv {} ...'.format(venv_path))
            mkdirs_verbose(venv_path)
        ...
        # install neovim & jedi
        print('Installing neovim, jedi ...')
        install_cmd = [ pip_exec,
                        'install', 'neovim', 'jedi' ]
        if self.user_env.cert is not None:
    ...
    def _setup_venvs(self):
        # Python3
        self._setup_venv(self.user_env.python2_venv, self.user_env.python2)
        self._setup_venv(self.user_env.python3_venv, self.user_env.python3)
    ...
    def _create_init_vim(self):
        ...
        with open(init_vim_file, 'w') as f:
            f.write('let g:python_host_prog="{}"\n'.format(
                        os.path.join(self.user_env.python2_venv, 'bin', 'python')))
            f.write('let g:python3_host_prog="{}"\n'.format(
                        os.path.join(self.user_env.python3_venv, 'bin', 'python3')))
